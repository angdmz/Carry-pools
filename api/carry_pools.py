...
@router.post("/vesting-schedules/{recharge_id}/satisfy", status_code=http.HTTPStatus.ACCEPTED)
...
@router.patch("/vesting-schedules/{recharge_id}/reject", status_code=http.HTTPStatus.ACCEPTED)
async def satisfy_recharge(recharge_id: UUID = Path(..., description="Recharge ID"),
                             session: AsyncSession = Depends(get_session)):
    async with session.begin():
        waiting_recharge = await RetrievedWaitingRecharge.from_persistance(recharge_id, session)
